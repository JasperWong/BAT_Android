...
public class CircleSeekBar extends View{
    ...
    private void initViewAttrs(AttributeSet attrs){
        ...
        int progressBackgroundColor = localTypedArray.getColor(R.styleable.CircleSeekBar_progress_background, Color.WHITE);
    ...
    private void initViewDefault(AttributeSet attrs){
        TypedArray localTypedArray = mContext.obtainStyledAttributes(attrs, R.styleable.CircleSeekBar);
        ...
        int progressBackgroundColor = Color.WHITE;
    ...
    private void seekTo(float eventX, float eventY, boolean isUp) {
        if (true == isPointOnThumb(eventX, eventY) && false == isUp) {
            ...
            int tempCurrent = (int) (mSeekBarMax * mSeekBarDegree / 360);
            if(Math.abs( mCurrentProgress - mLastProgress ) < 90 )
            {
                setThumbPosition(radian);
                mCurrentProgress = tempCurrent;
